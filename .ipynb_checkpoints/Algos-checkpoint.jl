...
function tune_meta(Table)
    (K,T)=size(Table)
    ChosenArms = zeros(Int,T)
    evict = [1,0.1,0.01,0.001,0.0001]
    replay = [10,1,0.1,0.01,0.001,0.0001]
    upper_prob = [0.5,0.8,0.9]
    current_min = T
    params = [1,10,0.5]
    for e in evict
        for r in replay
            for u in upper_prob
                ChosenArms,episode,ChangePoints= META(Table,e,r,u,"off")
                reg = ComputeCumRegret(Table,ChosenArms)
                if reg[T] < current_min
                    current_min = reg[T]
                    params = [e,r,u]
                end
            end
